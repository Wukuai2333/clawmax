...
assert(source.includes("fragment.get('agentforge_enrollment')"), 'AgentForge must consume the automatic enrollment handoff')
assert(source.includes('with no code or API key to copy'), 'AgentForge setup must explain the automatic participant flow')
assert(!source.includes('aria-label="AgentForge connection code"'), 'participants must not enter enrollment codes manually')
...
console.log('ActivityExportDeliveryStatus.test.ts: 13 assertions passed')
